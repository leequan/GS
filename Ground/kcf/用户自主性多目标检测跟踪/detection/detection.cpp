...
box Detection::select_obj_box(detection_info **info, int num)
{
    ...
    selected_box.w = info[max_index]->right - info[max_index]->left;
    selected_box.h = info[max_index]->bottom - info[max_index]->top;
...
// detect multi-targets
int Detection::detect_image(Mat frame, char *frame_file, double thresh, double hier_thresh, box* obj_rect)
{
    int num = 0;                        // the number of detected boxes
    float *feature_map = NULL;
    int map_size = 0;
    ...
    if(num != 0){
        for(int i = 0; i < num; i++){
            obj_rect[i].x = info[i]->left;
            obj_rect[i].y = info[i]->top;
            obj_rect[i].w = info[i]->right - info[i]->left + 1;
            obj_rect[i].h = info[i]->bottom - info[i]->top + 1;
        }
    }
    return num;
}
