...
box Detection::select_obj_box(detection_info **info, int num)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        cout << info[i]->left << ", " << info[i]->right << endl;
...
// detect with disk frame file
box Detection::detect_image(Mat frame, Mat obj_img, char *frame_file, double thresh, double hier_thresh, int xx, int yy, int &detect_flag)
{
    int num = 0;                        // the number of detected boxes
    float *feature_map = NULL;
    int map_size = 0;
    ...
    if (num == 0) {
     ...
     // obj_box = select_obj_box(info, num);
    obj_box = select_obj_box_all(frame, obj_img, info, num,xx,yy,detect_flag);
